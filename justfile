...
help:
    @clear
    @echo ""
    @printf "\033[0;34m=== tmux-router ===\033[0m\n"
    @echo ""
    ...
    @printf "  %-40s %s\n" "run-tmux" "Launch in a new tmux-router session (fails if it exists)"
    @echo ""
...
init: check
    ...
    @uv run pre-commit install || { printf "\033[31m✗ Could not install git hooks\033[0m\n"; exit 1; }
...
# Launch the app in a new named tmux session
run-tmux:
    @echo ""
    @tmux new-session -d -s tmux-router -c {{quote(justfile_directory())}} just run || { printf "\033[31m✗ Could not create tmux-router session\033[0m\n"; exit 1; }
    @if [ -n "${TMUX-}" ]; then \
        tmux switch-client -t '=tmux-router'; \
    else \
        tmux attach-session -t '=tmux-router'; \
    fi || { printf "\033[31m✗ Could not enter tmux-router session\033[0m\n"; exit 1; }
    @printf "\033[32m✓ tmux-router launcher completed\033[0m\n"
    @echo ""
